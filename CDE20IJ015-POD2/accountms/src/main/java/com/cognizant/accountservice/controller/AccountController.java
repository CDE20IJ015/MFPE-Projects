...
public class AccountController {
	...
	public ResponseEntity<Account> updateAccount(@RequestHeader("Authorization") String token,@RequestBody Account acc) {
		accountServiceImpl.hasPermission(token);
		Account updatedAccount=accountServiceImpl.updateAccount(acc);
	...
	public ResponseEntity<?> checkAccountBalance(@RequestHeader("Authorization") String token,
			@PathVariable long accountId) {
		accountServiceImpl.hasPermission(token);
		double Balance = accountServiceImpl.getAccount(accountId).getCurrentBalance();
		log.info("CurrentBalance retrieved successfully");
